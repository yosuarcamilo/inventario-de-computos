...
try {
    if (isset($_POST['agregar_punto_ap'])) {
        ...
        $errores = $validador->validarPuntoAP($datos_punto);
        ...
        $stmt = $conn->prepare("INSERT INTO puntos_ap (sede_id, numero, ubicacion, estado) VALUES (?, ?, ?, ?)");
        if (!$stmt) {
        ...
        if ($stmt->execute()) {
            ...
            jsonResponse(true, "Punto AP agregado correctamente. ID: $inserted_id", ['redirect' => 'puntos_ap']);
        } else {
            jsonResponse(false, 'Error al agregar punto AP: ' . $stmt->error);
        }
        $stmt->close();
    }

    if (isset($_POST['actualizar_punto_ap'])) {
        $sede_id = isset($_POST['sede_id']) ? (int)$_POST['sede_id'] : 1;
        $id = (int)$_POST['id'];
        $numero = trim($_POST['numero']);
        $ubicacion = trim($_POST['ubicacion']);
        ...
        $errores = $validador->validarPuntoAP($datos_punto, $id);
        ...
        $stmt = $conn->prepare("UPDATE puntos_ap SET numero = ?, ubicacion = ?, estado = ? WHERE id = ? AND sede_id = ?");
        if (!$stmt) {
            jsonResponse(false, 'Error en la preparación de la consulta: ' . $conn->error);
        }
        ...
        if ($stmt->execute()) {
            jsonResponse(true, 'Punto AP actualizado correctamente.', ['redirect' => 'puntos_ap']);
        } else {
            jsonResponse(false, 'Error al actualizar punto AP: ' . $stmt->error);
        }
        $stmt->close();
    }

    if (isset($_POST['eliminar_punto_ap'])) {
        $sede_id = isset($_POST['sede_id']) ? (int)$_POST['sede_id'] : 1;
        $id = (int)$_POST['id'];

        $stmt = $conn->prepare("DELETE FROM puntos_ap WHERE id = ? AND sede_id = ?");
        if (!$stmt) {
            jsonResponse(false, 'Error en la preparación de la consulta: ' . $conn->error);
        }
        ...
        if ($stmt->execute()) {
            jsonResponse(true, 'Punto AP eliminado correctamente.', ['redirect' => 'puntos_ap']);
        } else {
            jsonResponse(false, 'Error al eliminar punto AP: ' . $stmt->error);
        }
        $stmt->close();
    }
