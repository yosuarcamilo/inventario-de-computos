...
$sql_ubicaciones = "SELECT DISTINCT ubicacion FROM puntos_regulares WHERE sede_id = '$sede_id' ORDER BY ubicacion";
...
// Obtener puntos regulares con filtros
...
$sql = "SELECT * FROM puntos_regulares WHERE $where_clause ORDER BY numero";
$result = $conn->query($sql);
?>

<div class="content-section">
    <div class="section-header">
        <h2><i class="fas fa-plug"></i> Puntos Regulares</h2>
        <p>Gestiona los puntos regulados de la red en las diferentes ambientes</p>
    </div>
    ...
    <!-- Botón para agregar punto regular -->
    <div class="action-bar">
        <button class="btn-primary" onclick="abrirModalAgregar()">
            <i class="fas fa-plus"></i> Agregar Punto regulado
        </button>
        <button class="btn-success" onclick="exportarPuntosRegulares()">
    ...
    <!-- Tabla de puntos regulares -->
    <div class="table-container">
        <table class="data-table" id="tablaPuntosRegulares">
            <thead>
                <tr>
                    <th>Número</th>
                    <th>Ubicación</th>
            ...
            <tbody>
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td class="actions">
                                <button class="btn-view" onclick="verPuntoRegular(<?php echo $row['id']; ?>)">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button class="btn-edit" onclick="editarPuntoRegular(<?php echo $row['id']; ?>)">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn-delete" onclick="eliminarPuntoRegular(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars($row['numero']); ?>')">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="no-data">No hay puntos regulados registrados</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<!-- Modal para ver punto regular -->
<div id="modalVerPunto" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Ver Punto Regular</h3>
...
<!-- Modal para editar punto regular -->
<div id="modalEditarPunto" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Editar Punto Regular</h3>
            <span class="close" onclick="cerrarModalEditar()">&times;</span>
        </div>
        
        <form id="formEditarPunto" class="modal-form">
            <input type="hidden" name="actualizar_punto_regular" value="1">
            <input type="hidden" name="sede_id" value="<?php echo $sede_id; ?>">
...
<div id="modalConfirmarEliminar" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <p><i class="fas fa-exclamation-triangle" style="color: #e74c3c; margin-right: 10px;"></i>¿Estás seguro de que quieres eliminar este punto regular?</p>
            <p><strong>⚠️ Esta acción no se puede deshacer.</strong></p>
        </div>
        
        <form id="formEliminar">
            <input type="hidden" name="eliminar_punto_regular" value="1">
            <input type="hidden" name="sede_id" value="<?php echo $sede_id; ?>">
...
function recargarTabla() {
    ...
    if (typeof loadContent === 'function') {
        loadContent('puntos_regulares', sede_id);
    } else {
        // Fallback: recargar la página completa
        window.location.href = `../admin/index.php?page=puntos_regulares&sede_id=${sede_id}`;
    }
}
...
document.getElementById('formAgregarPunto').addEventListener('submit', function(e) {
    ...
    fetch('../controller/puntos_regulares_controller.php', {
        method: 'POST',
        body: formData
    })
...
// Ver punto regular
function verPuntoRegular(id) {
    fetch(`../controller/obtener_punto_regular.php?id=${id}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                mostrarMensaje('Error al obtener los datos del punto regular', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
...
// Editar punto regular
function editarPuntoRegular(id) {
    fetch(`../controller/obtener_punto_regular.php?id=${id}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
// Eliminar punto regular
function eliminarPuntoRegular(id, numero) {
...
function filtrarTiempoReal() {
    ...
    const tabla = document.querySelector('#tablaPuntosRegulares tbody');
...
// Función para exportar puntos regulares
function exportarPuntosRegulares() {
    ...
    let url = `../controller/exportar_puntos_regulares.php?sede_id=${sedeId}`;
